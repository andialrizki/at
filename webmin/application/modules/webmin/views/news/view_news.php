              ...
              <tbody>
                ...
                <?php foreach ($data->result() as $d): ?>
                  <tr>
                    ...
                    <td><?php echo checkStatus($d->news_status) ?></td>
                    <td>
                      <a class="btn btn-success btn-xs" onclick="formedit('<?php echo $d->news_id ?>', '<?php echo $d->news_title ?>', '<?php echo $d->news_category_id ?>', '<?php echo $d->news_image ?>', '<?php echo $d->news_status ?>');">Edit</a> 
                      <a href="<?php echo webmin_url('news/remove/'.$d->news_id) ?>" class="btn btn-danger btn-xs" onclick="return confirm('Are you sure to delete : <?php echo $d->news_title ?>? ');">Remove</a>
                    </td>
                  </tr>
                  <?php $no++ ?>
                <?php endforeach ?>
...
<div class="modal fade" id="modal-add">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <form class="form-horizontal" id="form" action="<?php echo webmin_url('news?action=save') ?>" method="post" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          <div class="form-group">
            ...
            <div class="col-sm-9">
              <input type="text" class="form-control" id="f1" name="post[news_title]" required>
            </div>
          </div>
          <div class="form-group">
            <label class="control-label col-sm-3">Category</label>
            <div class="col-sm-8">
              <select class="form-control" id="f2" name="post[news_category_id]" required>
              ...
              <div class="nav-tabs-custom">
                ...
                <div class="tab-content">
                  <div class="tab-pane active" id="tab_1">
                    <textarea id="ckeditor" name="post[news_desc_en]"></textarea>
                  </div>
                  <div class="tab-pane" id="tab_2">
                    <textarea id="ckeditor2" name="post[news_desc_id]"></textarea>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="form-group">
            ...
            <div class="col-sm-9">
              <select class="form-control" name="post[news_status]" id="f4" required>
                <option value="">-- select --</option>
...
<script type="text/javascript">
  ...
  function formedit(id, f1, f2, f3, f4) {
    $("#f1").val(f1);
    $("#f2").val(f2);
    $("#image img").attr('src', '<?php echo newsimg_url() ?>' + f3);
    $("#f4").val(f4);
    $("#modal-add #form").attr('action', $("#modal-add #form").attr('action') + '&id=' + id);
    $.ajax({
      url: '<?php echo webmin_url('news/getByAjax') ?>/' + id,
      cache: false,
      dataType: "json",
      success:function(data){
        tinymce.get('ckeditor').setContent(data.news_desc_en);
        tinymce.get('ckeditor2').setContent(data.news_desc_id);
      }
    });
