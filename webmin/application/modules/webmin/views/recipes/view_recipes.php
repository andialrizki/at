            ...
            <table class="table table-bordered datatable">
              ...
              <tbody>
                ...
                <?php foreach ($data->result() as $d): ?>
                  <tr>
                    <td><?php echo $no ?></td>
                    <td><?php echo $d->recipes_title ?></td>
                    <td><?php echo $d->category_name ?></td>
                    <td><?php echo checkStatus($d->recipes_status) ?></td>
                    <td>
                      <a class="btn btn-success btn-xs" onclick="formedit('<?php echo $d->recipes_id ?>', '<?php echo $d->recipes_title ?>', '<?php echo $d->recipes_category_id ?>', '<?php echo $d->recipes_image ?>', '<?php echo $d->recipes_status ?>');">Edit</a> 
                      <a href="<?php echo webmin_url('news/remove/'.$d->recipes_id) ?>" class="btn btn-danger btn-xs" onclick="return confirm('Are you sure to delete : <?php echo $d->recipes_title ?>? ');">Remove</a>
                    </td>
                  </tr>
                  <?php $no++ ?>
                <?php endforeach ?>
...
<div class="modal fade" id="modal-add">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <form class="form-horizontal" id="form" action="<?php echo webmin_url('recipes?action=save') ?>" method="post" enctype="multipart/form-data">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
          <h4 class="modal-title">Form Recipes</h4>
        </div>
        <div class="modal-body">
          <div class="form-group">
            <label class="control-label col-sm-3">Recipes Title</label>
            <div class="col-sm-9">
              <input type="text" class="form-control" id="f1" name="post[recipes_title]" required>
            </div>
          </div>
          <div class="form-group">
            <label class="control-label col-sm-3">Category</label>
            <div class="col-sm-8">
              <select class="form-control" id="f2" name="post[recipes_category_id]" required>
              ...
              <div class="nav-tabs-custom">
                ...
                <div class="tab-content">
                  <div class="tab-pane active" id="tab_1">
                    <textarea id="ckeditor" name="post[recipes_desc_en]"></textarea>
                  </div>
                  <div class="tab-pane" id="tab_2">
                    <textarea id="ckeditor2" name="post[recipes_desc_id]"></textarea>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="form-group">
            ...
            <div class="col-sm-9">
              <select class="form-control" name="post[recipes_status]" id="f4" required>
                <option value="">-- select --</option>
...
<script type="text/javascript">
  ...
  function formedit(id, f1, f2, f3, f4) {
    $("#f1").val(f1);
    $("#f2").val(f2);
    $("#image img").attr('src', '<?php echo recipesimg_url() ?>' + f3);
    $("#f4").val(f4);
    $("#modal-add #form").attr('action', $("#modal-add #form").attr('action') + '&id=' + id);
    $.ajax({
      url: '<?php echo webmin_url('recipes/getByAjax') ?>/' + id,
      cache: false,
      dataType: "json",
      success:function(data){
        tinymce.get('ckeditor').setContent(data.recipes_desc_en);
        tinymce.get('ckeditor2').setContent(data.recipes_desc_id);
